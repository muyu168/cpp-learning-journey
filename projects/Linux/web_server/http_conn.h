...
class http_conn{
    public:
        ...
        static int m_epollfd;//所有socket上的事件都被注册达到同一个epoll对象中
    ...
    private:
        ...
        int m_read_idx;                         // 读缓存区中已经读取到的字节的下一个字节的索引
        int m_checked_index;                    //当前正在分析的字符在读缓冲区的位置
        int m_start_line;                       //当前正在解析的行的起始位置

        CHECK_STATE m_check_state;              //主状态机当前所处的位置
        char* m_url;                            //从HTTP请求中分析请求目标url
        char* m_version;                        //从HTTP请求中分析请求的版本号
        METHOD m_method;                        //从HTTP请求中分析请求方法
        char* m_host;                           //从HTTP请求中分析请求的主机名
